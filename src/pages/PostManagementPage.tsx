    ...
    <div className="min-h-screen p-3 sm:p-6 space-y-5" style={{ background: "#fff" }}>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="sm:hidden space-y-3">
            {posts.map((post) => {
              ...
              return (
                ...
                >
                  {/* Row 1: image + caption + delete */}
                  <div className="flex items-start gap-3">
                    {post.postType === "meal_plan" ? (
                      <div className="w-14 h-14 rounded-xl flex items-center justify-center shrink-0" style={{ background: "#f0f4ee" }}>
                        <Utensils size={20} style={{ color: "#89957F" }} />
                      </div>
                    ) : imgUrl ? (
                      <img src={imgUrl} alt="post" className="w-14 h-14 rounded-xl object-cover shrink-0"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {post.postType === "meal_plan" && (
                        <span className="inline-block mt-1 text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "#f0f4ee", color: "#89957F" }}>
                          Meal Plan
                        </span>
                      )}
                    </div>
                    <button
                      ...
                      className="p-2 rounded-xl text-red-400 hover:text-white hover:bg-red-500 transition-all disabled:opacity-40 shrink-0"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      <p className="text-black font-semibold text-[12px] truncate">{post.user?.name || "Unknown"}</p>
                      <p className="text-gray-400 text-[11px] truncate">{post.user?.email || "—"}</p>
                    </div>
                  </div>

                  {/* Row 3: topic + date + stats */}
                  <div className="flex items-center flex-wrap gap-2">
                    {post.topic ? (
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold text-white" style={{ background: post.topic.color || "#89957F" }}>
                        {post.topic.name}
                      </span>
                    ) : (
                      <span className="text-gray-300 text-xs">No topic</span>
                    )}
                    <span className="text-[11px] text-gray-500">{formatDate(post.createdAt)} · {formatTime(post.createdAt)}</span>
                    <span className="flex items-center gap-1 text-[12px] text-gray-500 ml-auto">
                      <Heart size={12} style={{ color: "#e57373" }} />{post.likeCount}
                    </span>
                    <span className="flex items-center gap-1 text-[12px] text-gray-500">
                      <MessageCircle size={12} style={{ color: "#89957F" }} />{post.commentCount}
                    </span>
                  </div>
                </div>
              );
          ...
          {/* Desktop table */}
          <div className="hidden sm:block overflow-x-auto rounded-2xl border" style={{ borderColor: "rgba(0,0,0,0.08)" }}>
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-[11px] font-bold uppercase tracking-[0.12em]"
                  style={{ background: "#f9fafb", borderBottom: "1px solid rgba(0,0,0,0.07)", color: "#6b7280" }}>
                ...
                {posts.map((post, idx) => {
                  ...
                  return (
                    <tr key={post.id} className="transition-colors hover:bg-gray-50"
                      style={{ borderBottom: idx < posts.length - 1 ? "1px solid rgba(0,0,0,0.05)" : "none" }}>
                      <td className="px-4 py-3">
                        {post.postType === "meal_plan" ? (
                          <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: "#f0f4ee" }}>
                            <Utensils size={20} style={{ color: "#89957F" }} />
                          </div>
                        ) : imgUrl ? (
                          <img src={imgUrl} alt="post" className="w-12 h-12 rounded-xl object-cover"
                            onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                        ) : (
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          <UserAvatar user={post.user} />
                          <div className="min-w-0">
                            <p className="text-black font-semibold text-[13px] truncate max-w-[140px]">{post.user?.name || "Unknown"}</p>
                            <p className="text-gray-400 text-[11px] truncate max-w-[140px]">{post.user?.email || "—"}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        {post.topic ? (
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold text-white whitespace-nowrap"
                            style={{ background: post.topic.color || "#89957F" }}>
                            {post.topic.name}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <p className="text-black text-[13px] font-medium">{formatDate(post.createdAt)}</p>
                        <p className="text-gray-400 text-[11px]">{formatTime(post.createdAt)}</p>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <span className="flex items-center gap-1 text-[12px] text-gray-500">
                            <Heart size={13} style={{ color: "#e57373" }} />{post.likeCount}
                          </span>
                          <span className="flex items-center gap-1 text-[12px] text-gray-500">
                            <MessageCircle size={13} style={{ color: "#89957F" }} />{post.commentCount}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-right">
                        <button onClick={() => setConfirmId(post.id)} disabled={deletingId === post.id}
